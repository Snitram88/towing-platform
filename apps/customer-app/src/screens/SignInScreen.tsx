...
type Props = NativeStackScreenProps<CustomerAuthStackParamList, 'SignIn'>;
...
export default function SignInScreen({ navigation }: Props) {
  ...
  const handleSignIn = async () => {
    ...
    Alert.alert('Welcome back', 'Your session is active now.');
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <LinearGradient colors={['#0B1220', '#0A2540', '#111827']} style={styles.hero}>
            ...
            <Text style={styles.heroEyebrow}>Customer sign in</Text>
            <Text style={styles.heroTitle}>Welcome back.</Text>
            <Text style={styles.heroSubtitle}>
              Sign in to continue booking premium towing services with live tracking.
            </Text>
          </LinearGradient>

          <View style={styles.formCard}>
            <Text style={styles.cardTitle}>Access your account</Text>
            ...
            <Pressable onPress={() => navigation.navigate('SignUp')} style={styles.footerAction}>
              <Text style={styles.footerActionText}>
                New here? <Text style={styles.footerActionStrong}>Create an account</Text>
              </Text>
            </Pressable>
          </View>
        </ScrollView>
